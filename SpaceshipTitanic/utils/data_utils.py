...
def initialize_dvc() -> None:
    ...
    run_shell_command("dvc add data/raw")
...
def initialize_dvc_storage(dvc_remote_name: str, dvc_remote_url: str) -> None:
    if not run_shell_command("dvc remote list"):
        ...
        run_shell_command("git commit -nm 'Initialized dvc remote storage at {dvc_remote_url}'")
    else:
...
def commit_to_dvc(dvc_raw_data_folder: str, dvc_remote_name: str) -> None:
    ...
    run_shell_command(f"git commit -nm 'Updated data version from {current_version} to {next_version}'")
...
def make_new_data_version(dvc_raw_data_folder: str, dvc_remote_name: str) -> None:
    try:
        status = run_shell_command(f"dvc status | grep {dvc_raw_data_folder}")
        if status == "Data and pipeline are up to date.\n":
            DATA_UTILS_LOGGER.info("Data is up to date.")
            return
